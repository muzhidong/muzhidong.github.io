...
> 产生闭包不等于闭包生效：如果内部函数在外部函数执行完毕后，没有返回被调用、没有赋值给全局变量，那么即使产生了闭包，外部函数的变量也会随着执行上下文出栈而被垃圾回收

> 通常讨论的闭包，是指内部函数在外部函数执行结束后仍然被使用的情况
...
## 编程范式
- 命令式编程：关注怎么做，聚焦步骤、状态变化。分结构化和非结构化，以下是结构化的再分类
  - 面向过程编程，如C、Go
  - 面向对象编程OOP：通过继承解决纵向的代码重复问题，如C++、Java、C#
  - 面向切面编程AOP：不是独立的编程范式，是面向对象编程的补充，解决横向的跨多个方法的重复逻辑。通过横向抽取机制，将分散在多个方法中的横切逻辑代码与业务逻辑分离。切是指操作横切逻辑，不侵入原有业务代码，面是指横切逻辑影响多个方法(点)，这些点构成一个面。特点是消除横切逻辑代码的重复，避免代码冗余；解耦业务逻辑和横切逻辑，提升代码可维护性。可用于日志埋点、权限检查等。示例如下，
  ```javascript
...
- 声明式编程：关注做什么，聚焦结果。分函数式和逻辑式
  - 函数式编程FP：无副作用，适合分布式计算集群的应用场景，但过度使用会导致可读性稍弱差、学习难度大（掌握λ演算、代数、范畴学等基础）。如Lisp

> 现代主流语言，如JavaScript、Python、Scala、Kotlin，大多是多范式语言，它们既支持命令式写法，也支持函数式特性
