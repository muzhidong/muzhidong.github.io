...
## 1、构建DOM
HTML文档逐步从字节、字符、语义块到节点，形成DOM。
![](/browser/render_engine/构建DOM_1.jpg)

过程可分为以下三步，
- 字符转换：浏览器从磁盘或网络读取HTML原始字节，并根据文件编码转换成字符
- token化(词法分析)：浏览器通过状态机从字符解析出W3C标准规定的各种token，每个token具有独特含义和一组规则。之后token会转换成定义其属性和规则的对象
  ![](/browser/render_engine/构建DOM_2.jpg)
- 构建DOM：根据不同标记间的关系，通过栈构建DOM树
...
## 3、样式计算
- 解析CSS​获取一组组样式规则
- 从DOM树根节点开始，遍历每个可见节点(不可见节点或通过CSS隐藏的节点都会被忽略)，将CSS规则应用到DOM树的每个节点上，根据选择器特性、优先级、权重，计算出最终的样式规则

> 为什么最终生成的CSS OM是树结构？
>
> 一是便于与DOM结构一一对应​，二是配合CSS特性，支持继承，处理上下文，三是优化性能，实现局部样式计算
