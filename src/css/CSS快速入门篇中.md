...
- 元素按是否具有可替换内容分类
  ...
  **替换元素**
  ...
  - 2、有自己的默认尺寸。如video为640px * 480px、img为0、表单的默认尺寸与浏览器有关
  - 3、在很多CSS属性上有自己的一套表现规则，比如vertical-align属性默认值是baseline，但在替换元素中，该值被视为为元素的下边缘
  
  > <div id="size">尺寸计算规则优先级：默认尺寸 < 固有尺寸（替换内容原本的尺寸） < HTML尺寸（通过HTML属性改变的尺寸） < CSS尺寸（通过CSS属性改变的content-box尺寸）</div>
  > 
  > 解惑：
  > - 1、块级替换元素与内联替换元素的尺寸计算规则表现一致。这也是为什么图片或表单替换元素设置display:block而宽度没有100%容器的原因
  > - 2、固有尺寸是不可改变的，比如canvas固有尺寸是300px*150px。至于为什么图片CSS尺寸设置了会覆盖固有尺寸，因为img图片标签内容默认适配方式是fill。在CSS3可以通过object-fit更改替换元素内容的适配方式
